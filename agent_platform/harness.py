...
class WorkerResult:
    ...
    def as_dict(self) -> Dict[str, Any]:
        return {"worker_id": self.worker_id, "status": self.status, "result": self.result, "error": self.error, "execution_id": self.execution_id, "session_id": self.session_id}
...
class CollaborationResult:
    ...
    def as_dict(self) -> Dict[str, Any]:
        return {"task_id": self.task_id, "status": self.status, "workers": [worker.as_dict() for worker in self.workers]}

class CollaborationHarness:
    """Run isolated Yasin-Agent workers concurrently with bounded concurrency."""
    def __init__(self, *, runtime: Optional[ExecutionRuntime] = None, max_concurrent_workers: int = 4) -> None:
    ...
    def register(self, worker_id: str, runner: WorkerCallable, *, agent_id: Optional[str] = None, capabilities: Sequence[str] = (), workspace: Any = None, metadata: Optional[Mapping[str, Any]] = None) -> WorkerSpec:
        worker = WorkerSpec(worker_id=worker_id, runner=runner, agent_id=agent_id, capabilities=tuple(capabilities), workspace=workspace, metadata=dict(metadata or {}))
    ...
    def run(self, task_id: str, *, context: Optional[Mapping[str, Any]] = None, worker_ids: Optional[Sequence[str]] = None, max_concurrent_workers: Optional[int] = None) -> CollaborationResult:
        with self._lock:
        ...
        def execute(spec: WorkerSpec) -> WorkerResult:
            ...
            execution = self.runtime.create(task_id=task_id, agent_id=spec.agent_id or spec.worker_id, workspace=spec.workspace, capabilities=tuple(spec.capabilities), metadata={**dict(parent_context), **dict(spec.metadata), "worker_id": spec.worker_id})
            with self._lock:
                self._executions[task_id][spec.worker_id] = execution.execution_id
            self.runtime.events.emit("worker.registered", execution_id=execution.execution_id, task_id=task_id, session_id=execution.session_id, status=execution.status.value, agent_id=execution.agent_id, workspace_id=execution.workspace.workspace_id, metadata={"worker_id": spec.worker_id})
        ...
        try:
            with ThreadPoolExecutor(max_workers=limit, thread_name_prefix="yasin-worker") as pool:
                futures: Dict[Future[WorkerResult], str] = {pool.submit(execute, spec): spec.worker_id for spec in specs}
...
__all__ = ["CollaborationHarness", "CollaborationResult", "WorkerResult", "WorkerSpec"]
