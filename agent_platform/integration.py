"""
integration.py
Yasin-Agent integration with Yasin-Core public SDK only.

Adapters and helpers so agent_platform agents run via YasinCoreClient.
Consumers must import only from yasin_core.sdk — never Core internals.
"""
...
class MockContext:
    """Local context fallback when no Core client is available."""
            ...
            for filename in os.listdir(plugins_dir):
                ...
                try:
                    ...
                    for name, obj in local_dict.items():
                        if not isinstance(obj, type) or obj is YasinPlugin:
                            continue
                        is_plugin = False
                        try:
                            if issubclass(obj, YasinPlugin):
                                is_plugin = True
                        except TypeError:
                            pass
                        if not is_plugin and hasattr(obj, "name") and (
                            hasattr(obj, "execute") or hasattr(obj, "run")
                        ):
                            is_plugin = True
                        if is_plugin:
                            plugin_inst = obj()
                            if not hasattr(plugin_inst, "name"):
                                plugin_inst.name = getattr(obj, "name", name)
...
class YasinCoreAgentAdapter(BaseAgent):
    """Adapt an agent_platform agent as a Core BaseAgent."""
    ...
    def execute(self, input_data: Dict[str, Any]) -> Any:
        ...
        core_context.set(
            "task_metadata",
            {"agent_name": self.name, "goal": self.agent_config.goal},
        )

        from agent_platform.agent_definition import AgentDefinition

        if isinstance(self.agent_config, AgentDefinition):
            system_prompt = self.agent_config.prompt_handler.render_system_prompt(
                self.agent_config.profile
            )
...
def execute_plugin_via_sdk(
    client: YasinCoreClient, plugin_name: str, input_data: Dict[str, Any]
) -> Any:
